...
module NidCorrectionPortalUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NidCorrectionPortalResponse.new({})
    ...
    if fetch_err
      ...
      response = NidCorrectionPortalResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NidCorrectionPortalResponse.new(fetched)
    else
