...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("NID_CORRECTION_PORTAL_TEST_LIVE") == "TRUE" ||
		os.Getenv("NID_CORRECTION_PORTAL_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("NID_CORRECTION_PORTAL_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.NidCorrectionPortalSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.NidCorrectionPortalError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.NidCorrectionPortalSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.NidCorrectionPortalError{Msg: msg, Code: code}
}
