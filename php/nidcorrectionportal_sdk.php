...
class NidCorrectionPortalSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = NidCorrectionPortalHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = NidCorrectionPortalHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!NidCorrectionPortalFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, NidCorrectionPortalFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return NidCorrectionPortalUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = NidCorrectionPortalHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = NidCorrectionPortalHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = NidCorrectionPortalHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new NidCorrectionPortalError($op . "_allow",
                "NidCorrectionPortalSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = NidCorrectionPortalHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new NidCorrectionPortalError("graphql_error",
                "NidCorrectionPortalSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function CorrectionRequest($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_correction_request === null) {
                $this->_correction_request = new CorrectionRequestEntity($this, null);
            }
            return $this->_correction_request;
        }
        return new CorrectionRequestEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new NidCorrectionPortalSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
