...
from nidcorrectionportal_sdk.nidcorrectionportal_types import (
    CorrectionRequest,
...
class CorrectionRequestEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return CorrectionRequestEntity(self._client, opts)
    ...
    def data_get(self) -> CorrectionRequest:
    ...
    def match_get(self) -> CorrectionRequest:
    ...
    def load(self, reqmatch=None, ctrl=None) -> CorrectionRequest:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.CorrectionRequest().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[CorrectionRequest]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.CorrectionRequest().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
