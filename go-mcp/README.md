# nid-correction-portal-mcp

[MCP](https://modelcontextprotocol.io) server exposing the NidCorrectionPortal SDK as
two agent tools — `nid-correction-portal_list` and `nid-correction-portal_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nid-correction-portal-mcp)
make build

# 2. Provide credentials via the environment
export NID_CORRECTION_PORTAL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nid-correction-portal \
  -- /absolute/path/to/nid-correction-portal-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nid-correction-portal-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nid-correction-portal_list: first page of records
{ "entity": "correction_request" }
{ "entity": "correction_request", "query": { } }

// nid-correction-portal_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nid-correction-portal-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nid-correction-portal \
     -- "$PWD"/dist/*/nid-correction-portal-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nid-correction-portal_list` and `nid-correction-portal_load` tools now appear
   in new sessions. Ask the agent to *"list correction_request using nid-correction-portal"*
   and it calls `nid-correction-portal_list` with `{"entity":"correction_request"}`.
...
```sh
export NID_CORRECTION_PORTAL_APIKEY=sk_live_xxx            # API key
export NID_CORRECTION_PORTAL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `nid-correction-portal_list` tool
...
### Call the `nid-correction-portal_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `nid-correction-portal_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nid-correction-portal_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NID_CORRECTION_PORTAL_APIKEY` | API key sent with every request. |
| `NID_CORRECTION_PORTAL_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

application | authentication | correction_request

### Smoke test via HTTP (raw JSON-RPC)

```sh
./nid-correction-portal-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nid-correction-portal_load","arguments":{"entity":"application","query":{"id":1}}}}'
```
