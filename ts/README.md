# NidCorrectionPortal TypeScript SDK



The TypeScript SDK for the NidCorrectionPortal API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Application()` — each with a small set of operations (`list`, `load`, `create`)
...
```ts
const client = new NidCorrectionPortalSDK({ apikey: '...' })
...
```ts
new NidCorrectionPortalSDK(options?: {
  apikey?: string
  base?: string
  prefix?: string
  suffix?: string
...
| Method | Returns | Description |
| --- | --- | --- |
| `NidCorrectionPortalSDK.test(testopts?, sdkopts?)` | `NidCorrectionPortalSDK` | Create a test-mode client. |
...
├── src/
│   ├── NidCorrectionPortalSDK.ts        # Main SDK class
