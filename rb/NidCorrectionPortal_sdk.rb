...
require_relative 'NidCorrectionPortal_types'
...
class NidCorrectionPortalSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NidCorrectionPortalHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NidCorrectionPortalHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NidCorrectionPortalFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NidCorrectionPortalUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NidCorrectionPortalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NidCorrectionPortalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NidCorrectionPortalHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => NidCorrectionPortalError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue NidCorrectionPortalError => err
    ...
    if fetched.is_a?(Hash)
      status = NidCorrectionPortalHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NidCorrectionPortalError.new(
        "graphql_error", "NidCorrectionPortalSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NidCorrectionPortalSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
