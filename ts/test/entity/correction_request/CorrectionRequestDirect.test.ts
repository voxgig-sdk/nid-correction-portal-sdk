...
describe('CorrectionRequestDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new NidCorrectionPortalSDK({
  ...
  test('direct-load-correction_request', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-correction_request', setup.live)) return
  ...
  test('direct-list-correction_request', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-correction_request', setup.live)) return
    const { client, calls } = setup
