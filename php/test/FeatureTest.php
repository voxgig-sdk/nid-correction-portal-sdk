...
// NidCorrectionPortal SDK feature test
//
...
class FtCtrl extends NidCorrectionPortalControl
{
...
class FtClient
{
    ...
    private NidCorrectionPortalUtility $utility;

    public function __construct(NidCorrectionPortalUtility $utility, array $options)
    {
    ...
    public function get_utility(): NidCorrectionPortalUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NidCorrectionPortalContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NidCorrectionPortalContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NidCorrectionPortalContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NidCorrectionPortalConfig::make_feature($name);
    ...
    private static function buildUrl(NidCorrectionPortalSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NidCorrectionPortalError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NidCorrectionPortalResponse($fetched);
        }
    ...
    private function fail(NidCorrectionPortalContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NidCorrectionPortalContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
