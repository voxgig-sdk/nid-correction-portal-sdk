...
class ApplicationDirectTest < Minitest::Test
  def test_direct_load_application
    setup = application_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-application", setup[:live] ? "live" : "unit")
    if _should_skip
...
def application_direct_setup(mockres)
