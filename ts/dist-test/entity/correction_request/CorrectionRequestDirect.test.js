...
(0, node_test_1.describe)('CorrectionRequestDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.NidCorrectionPortalSDK({
    ...
    (0, node_test_1.test)('direct-load-correction_request', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-correction_request', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-correction_request', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-correction_request', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.NID_CORRECTION_PORTAL_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.NidCorrectionPortalSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
            apikey: env.NID_CORRECTION_PORTAL_APIKEY,
        }));
        let idmap = env['NID_CORRECTION_PORTAL_TEST_CORRECTION_REQUEST_ENTID'];
    ...
    const client = new __1.NidCorrectionPortalSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=CorrectionRequestDirect.test.js.map