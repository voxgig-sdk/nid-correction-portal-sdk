# NidCorrectionPortal SDK test runner
...
class NidCorrectionPortalTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NidCorrectionPortalTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NidCorrectionPortalTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NidCorrectionPortalTestRunner.getenv("NID_CORRECTION_PORTAL_TEST_LIVE")
        override = NidCorrectionPortalTestRunner.getenv("NID_CORRECTION_PORTAL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NidCorrectionPortalTestRunner.getenv(key)
        ...
        explain = NidCorrectionPortalTestRunner.getenv("NID_CORRECTION_PORTAL_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if NidCorrectionPortalTestRunner._test_control is not None:
            return NidCorrectionPortalTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NidCorrectionPortalTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NidCorrectionPortalTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NidCorrectionPortalTestRunner.load_test_control()
...
def load_env_local():
    NidCorrectionPortalTestRunner.load_env_local()


def env_override(m):
    return NidCorrectionPortalTestRunner.env_override(m)


def entity_data(v):
    return NidCorrectionPortalTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NidCorrectionPortalTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NidCorrectionPortalTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NidCorrectionPortalTestRunner.load_test_control()


def live_client_options():
    return NidCorrectionPortalTestRunner.live_client_options()


def live_delay_ms():
    return NidCorrectionPortalTestRunner.live_delay_ms()
