...
//    only - a typed NidCorrectionPortalContext there would make every ctx
...
final class NidCorrectionPortalOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NidCorrectionPortalContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new NidCorrectionPortalSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new NidCorrectionPortalResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new NidCorrectionPortalError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new NidCorrectionPortalResponse($clean['response']);
    ...
    public static function err_from_map($m): ?NidCorrectionPortalError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NidCorrectionPortalError(is_string($code) ? $code : '', $msg);
    }
