...
func TestAuthenticationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set NID_CORRECTION_PORTAL_TEST_AUTHENTICATION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func authenticationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NID_CORRECTION_PORTAL_TEST_AUTHENTICATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NID_CORRECTION_PORTAL_TEST_AUTHENTICATION_ENTID"])
