...
// NidCorrectionPortal SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NidCorrectionPortalTestFeature extends NidCorrectionPortalBaseFeature
{
    ...
    public function init(NidCorrectionPortalContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NidCorrectionPortalContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NidCorrectionPortalParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NidCorrectionPortalContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NidCorrectionPortalContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NidCorrectionPortalParam::call($ctx, $k);
