...
class CorrectionRequestDirectTest < Minitest::Test
  def test_direct_list_correction_request
    setup = correction_request_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-correction_request", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_correction_request
    setup = correction_request_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-correction_request", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def correction_request_direct_setup(mockres)
