...
class CorrectionRequestEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = NidCorrectionPortalSDK::test($seed, null);
        $seen = iterator_to_array($base->CorrectionRequest(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->CorrectionRequest(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NID_CORRECTION_PORTAL_TEST_CORRECTION_REQUEST_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $correction_request_ref01_data_dt0_load_result = Helpers::to_map(is_object($correction_request_ref01_data_dt0_loaded) && method_exists($correction_request_ref01_data_dt0_loaded, 'data_get') ? $correction_request_ref01_data_dt0_loaded->data_get() : $correction_request_ref01_data_dt0_loaded);
...
function correction_request_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("NID_CORRECTION_PORTAL_TEST_CORRECTION_REQUEST_ENTID");
