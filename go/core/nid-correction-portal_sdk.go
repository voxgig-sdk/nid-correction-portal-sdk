...
type NidCorrectionPortalSDK struct {
	Mode     string
...
func (sdk *NidCorrectionPortalSDK) OptionsMap() map[string]any {
...
func (sdk *NidCorrectionPortalSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *NidCorrectionPortalSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *NidCorrectionPortalSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *NidCorrectionPortalSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *NidCorrectionPortalSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *NidCorrectionPortalSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("NidCorrectionPortalSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *NidCorrectionPortalSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *NidCorrectionPortalSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("NidCorrectionPortalSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
...
func TestSDK(testopts map[string]any, sdkopts map[string]any) *NidCorrectionPortalSDK {
	...
	sdk := NewNidCorrectionPortalSDK(sdkopts)
	sdk.Mode = "test"
