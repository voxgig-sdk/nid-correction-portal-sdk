...
// NidCorrectionPortal SDK pipeline test
//
...
{
    private static function utility(): NidCorrectionPortalUtility
    {
        return new NidCorrectionPortalUtility();
    }
    ...
    private static function ctx(array $over = []): NidCorrectionPortalContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NidCorrectionPortalContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NidCorrectionPortalOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NidCorrectionPortalError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NidCorrectionPortalOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NidCorrectionPortalMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NidCorrectionPortalOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NidCorrectionPortalMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NidCorrectionPortalOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NidCorrectionPortalMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NidCorrectionPortalSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NidCorrectionPortalMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NidCorrectionPortalResponse([]),
            'result' => new NidCorrectionPortalResult([]),
        ]);
        [, $err] = NidCorrectionPortalMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NidCorrectionPortalSpec([]),
            'response' => null,
            'result' => new NidCorrectionPortalResult([]),
        ]);
        [, $err] = NidCorrectionPortalMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NidCorrectionPortalSpec(['step' => 's']),
            'response' => new NidCorrectionPortalResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NidCorrectionPortalResult([]),
        ]);
        [, $err] = NidCorrectionPortalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NidCorrectionPortalSpec(['step' => 's']),
            'response' => new NidCorrectionPortalResponse(self::resp(200, ['v' => 1])),
            'result' => new NidCorrectionPortalResult([]),
        ]);
        [, $err] = NidCorrectionPortalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NidCorrectionPortalSpec(['step' => 's']),
            'response' => new NidCorrectionPortalResponse(self::resp(200, ['v' => 2])),
            'result' => new NidCorrectionPortalResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NidCorrectionPortalMakeResponse::call($ctx);
    ...
    {
        $preset = new NidCorrectionPortalResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NidCorrectionPortalSpec([]),
            'response' => new NidCorrectionPortalResponse([]),
            'result' => new NidCorrectionPortalResult([]),
        ...
        [$got, $err] = NidCorrectionPortalMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NidCorrectionPortalResult([])]);
        [, $err] = NidCorrectionPortalMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NidCorrectionPortalSpec([]), 'result' => null]);
        [, $err] = NidCorrectionPortalMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NidCorrectionPortalSpec(['step' => 's']),
            'result' => new NidCorrectionPortalResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NidCorrectionPortalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NidCorrectionPortalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NidCorrectionPortalSpec(['step' => 's']),
            'result' => new NidCorrectionPortalResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NidCorrectionPortalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NidCorrectionPortalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NidCorrectionPortalResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NidCorrectionPortalSpec([]),
            'result' => new NidCorrectionPortalResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NidCorrectionPortalMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NidCorrectionPortalMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NidCorrectionPortalError('boom', 'boom');
        $utility->fetcher = function (NidCorrectionPortalContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NidCorrectionPortalSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NidCorrectionPortalMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NidCorrectionPortalContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NidCorrectionPortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NidCorrectionPortalResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        NidCorrectionPortalMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NidCorrectionPortalContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NidCorrectionPortalSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NidCorrectionPortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NidCorrectionPortalResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NidCorrectionPortalSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NidCorrectionPortalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NidCorrectionPortalMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NidCorrectionPortalMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NidCorrectionPortalResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NidCorrectionPortalDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NidCorrectionPortalResult(['ok' => false])]);
        $this->expectException(NidCorrectionPortalError::class);
        NidCorrectionPortalDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NidCorrectionPortalResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NidCorrectionPortalMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NidCorrectionPortalMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NidCorrectionPortalBaseFeature();
        $b = new NidCorrectionPortalBaseFeature();
        NidCorrectionPortalFeatureAdd::call($ctx, $a);
        NidCorrectionPortalFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NidCorrectionPortalBaseFeature
    {
        $f = new NidCorrectionPortalBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NidCorrectionPortalFeatureAdd::call($ctx, self::named_feature('a'));
        NidCorrectionPortalFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        NidCorrectionPortalFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        NidCorrectionPortalFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        NidCorrectionPortalFeatureAdd::call($ctx, $replace);
        ...
        NidCorrectionPortalFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NidCorrectionPortalContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NidCorrectionPortalSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NidCorrectionPortalPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        NidCorrectionPortalPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NidCorrectionPortalResponse(['headers' => null]),
            'result' => new NidCorrectionPortalResult([]),
        ]);
        NidCorrectionPortalResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        NidCorrectionPortalResultBody::call($ctx);
