...
// NidCorrectionPortal SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = NidCorrectionPortalOmni::makeStructRunner(
    ...
    {
        return NidCorrectionPortalOmni::objmodel(NidCorrectionPortalOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return NidCorrectionPortalOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return NidCorrectionPortalOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(NidCorrectionPortalOmni::structfix($now, Struct::undef()))
                        !== json_encode(NidCorrectionPortalOmni::structfix($val, Struct::undef()))) {
