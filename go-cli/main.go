// Command NidCorrectionPortal-cli is a CLI + REPL for the NidCorrectionPortal SDK,
...
const prompt = "nid-correction-portal"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "application authentication correction_request"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NID_CORRECTION_PORTAL_APIKEY (api key), NID_CORRECTION_PORTAL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NID_CORRECTION_PORTAL_APIKEY carries the API key and
	// NID_CORRECTION_PORTAL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NID_CORRECTION_PORTAL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NID_CORRECTION_PORTAL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNidCorrectionPortalSDK(opts)
