...
import (
	...
	omni "github.com/voxgig-sdk/nid-correction-portal-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NidCorrectionPortalError); is {
				out := map[string]any{
					"name":    "NidCorrectionPortalError",
...
func omniCtx(arg any, client *sdk.NidCorrectionPortalSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NidCorrectionPortalSDK); is {
			client = live
