...
The Lua SDK for the NidCorrectionPortal API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Application(nil):load({ id = "example_id" }, nil)
...
local result, err = client:NidCorrectionPortal(nil):load(
  { id = "test01" }, nil
)
