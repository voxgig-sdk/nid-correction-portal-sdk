# NidCorrectionPortal SDK Application entity
...
class ApplicationEntity
  ...
  def data_set(args)
    if args
      @_data = NidCorrectionPortalHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = NidCorrectionPortalHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [ApplicationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Application.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Application, Hash] the loaded Application; raises NidCorrectionPortalError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NidCorrectionPortalHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Application, Hash] the created Application; raises NidCorrectionPortalError on failure
