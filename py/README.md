# NidCorrectionPortal Python SDK

The Python SDK for the NidCorrectionPortal API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Application(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.NidCorrectionPortal(None).load(
    {"id": "test01"}, None
)
...
```python
client = NidCorrectionPortalSDK.test(testopts, sdkopts)
```
...
The main module (`nidcorrectionportal_sdk`) exports the SDK class.
