...
//     (\`new NidCorrectionPortalSDK(...)\` / \`NidCorrectionPortalSDK.test(...)\` become
//     \`NidCorrectionPortalSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NidCorrectionPortalSDK'
...
const TEST_SEED = {"entity":{"application":{"test01":{"id":"test01"}},"authentication":{"test01":{"id":"test01"}},"correction_request":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NidCorrectionPortalSDK(...)\` and any
// \`NidCorrectionPortalSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NidCorrectionPortalSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NidCorrectionPortalSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
