...
require("nid-correction-portal_types")
...
local NidCorrectionPortalSDK = {}
NidCorrectionPortalSDK.__index = NidCorrectionPortalSDK
...
NidCorrectionPortalSDK._make_feature = _make_feature


function NidCorrectionPortalSDK.new(options)
  local self = setmetatable({}, NidCorrectionPortalSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function NidCorrectionPortalSDK:options_map()
...
function NidCorrectionPortalSDK:get_utility()
  return Utility.copy(self._utility)
end


function NidCorrectionPortalSDK:get_root_ctx()
  return self._rootctx
end


function NidCorrectionPortalSDK:prepare(fetchargs)
...
function NidCorrectionPortalSDK:direct(fetchargs)
...
function NidCorrectionPortalSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function NidCorrectionPortalSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "NidCorrectionPortalSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function NidCorrectionPortalSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NidCorrectionPortalSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NidCorrectionPortalSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:CorrectionRequest():list() / client:CorrectionRequest():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
function NidCorrectionPortalSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = NidCorrectionPortalSDK.new(sdkopts)
  sdk.mode = "test"
