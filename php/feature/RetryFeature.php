...
// NidCorrectionPortal SDK retry feature
...
class NidCorrectionPortalRetryFeature extends NidCorrectionPortalBaseFeature
{
    ...
    public function init(NidCorrectionPortalContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NidCorrectionPortalContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NidCorrectionPortalContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NidCorrectionPortalContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
