# nid-correction-portal-cli

boru-driven command-line client **and** interactive REPL for the NidCorrectionPortal
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nid-correction-portal-cli)
make build

# 2. See usage (words, entities, env vars)
./nid-correction-portal-cli --help

# 3. Provide credentials once, via the environment
export NID_CORRECTION_PORTAL_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./nid-correction-portal-cli load 1 application            # {id:1} shorthand
./nid-correction-portal-cli load '{id:1}' application       # explicit match map

# 5. Override the API base URL for a single call
NID_CORRECTION_PORTAL_BASE=https://api.example.com ./nid-correction-portal-cli load 1 application

# 6. No arguments -> interactive REPL
./nid-correction-portal-cli
nid-correction-portal> load 1 application
nid-correction-portal> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nid-correction-portal-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nid-correction-portal-cli load 1 application
   ```
...
```sh
./nid-correction-portal-cli load 1 application          # scalar shorthand for {id:1}
./nid-correction-portal-cli load '{id:1}' application     # explicit match map
```
...
./nid-correction-portal-cli load 1 application
```
...
Run with no arguments to open a REPL (prompt `nid-correction-portal>`). Each line is
evaluated as its own boru expression:

```text
$ ./nid-correction-portal-cli
nid-correction-portal> load 1 application
nid-correction-portal> /help
nid-correction-portal> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nid-correction-portal-cli`. |
...
(`./nid-correction-portal-cli <expr>`) and interactively (the REPL), and expressions compose the
