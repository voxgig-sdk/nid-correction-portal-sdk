...
module NidCorrectionPortalFeatureHarness
  # True when this SDK was generated with the named feature.
  def self.has_feature?(name)
    f = NidCorrectionPortalConfig.shared_config["feature"]
  ...
  class Harness
    ...
    def initialize(features, server: nil, base: "http://api.test", headers: {})
      ...
      @utility.fetcher = server || NidCorrectionPortalFeatureHarness.default_server
      ...
      features.each do |fspec|
        name = fspec["name"]
        next unless NidCorrectionPortalFeatureHarness.has_feature?(name)
        f = NidCorrectionPortalFeatures.make_feature(name)
    ...
    def op(opname: "load", entity: "widget", method: nil, path: nil, query: nil,
      ...
      method ||= NidCorrectionPortalFeatureHarness.default_method(opname)
      ...
      begin
        fire(ctx, "PrePoint")
        raise ctx.out["point"] if ctx.out["point"].is_a?(NidCorrectionPortalError)
        ...
        ctx.response = fetched.is_a?(Hash) ? NidCorrectionPortalResponse.new(fetched) : nil
    ...
    def populate_result(ctx, fetched, fetch_err)
      result = NidCorrectionPortalResult.new({})
      ctx.result = result
...
class FeatureTest < Minitest::Test
  H = NidCorrectionPortalFeatureHarness
  ...
  def test_retry_retries_a_transport_error_tuple
    ...
    server, calls = H.recording_server { |n, _fd|
      n < 3 ? [nil, NidCorrectionPortalError.new("boom", "boom")] : [H.make_response(200, { "ok" => true }), nil]
    }
  ...
  def test_retry_exhausted_transport_error_surfaces
    skip_unless_feature("retry")
    clock = H::Clock.new
    server, calls = H.recording_server { |_n, _fd| [nil, NidCorrectionPortalError.new("boom", "boom")] }
    h = harness([fspec("retry",
      "retries" => 2, "minDelay" => 1, "jitter" => false, "sleep" => clock.sleeper)],
      server: server)
    res = h.op
